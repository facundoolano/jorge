...
The ~new~ command would be a helper to add create blog posts with some of the boilerplate (e.g. the front matter) already filled in. I eventually renamed this subcommand to ~post~ and dropped most of its options.
...
- This wasn't a strict plan I expected to follow but rather a list work I'd already identified, a guide to see where I needed to do more thinking, and another chance to validate my assumptions about the scope of the project.
